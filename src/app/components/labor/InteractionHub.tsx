...
export function InteractionHub({ caseId }: { caseId?: string }){
  const [msgs,setMsgs]=useState<Msg[]>([
    ...
    { id:'3', from:'موظف الرقابة', role:'compliance_officer', text:'قيد المراجعة — SLA باقي 2 يوم', at: new Date().toISOString() },
  ]);
  ...
  return (
    <Card>
      <div className="p-5 space-y-3">
        ...
        <div className="space-y-2 max-h-[260px] overflow-auto pr-1">
          {msgs.map(m=>(
            <div key={m.id} className={`p-3 border rounded-xl ${m.role==='me'?'bg-blue-50 border-blue-200':'bg-slate-50'}`}>
              <div className="flex items-center gap-2 text-xs text-muted-foreground"><MessageSquare className="w-3 h-3"/>{m.from} • {m.role} • {new Date(m.at).toLocaleTimeString('ar-YE')}</div>
        ...
        <div className="text-[11px] text-muted-foreground bg-slate-50 border rounded-lg p-2">كل رسالة تُسجل `audit + notification_event dedup_key` — لا تكرار، لا فقدان، مع `correlationId`</div>
      </div>
    </Card>
  );
}
