...
export async function decryptData(data: string, key?: string): Promise<string> {
    if (!data.startsWith('__ENCRYPTED__'))
        return data;
    try {
...
// ============================================================
export async function createIncrementalBackup(since: number, config: Partial<BackupConfig> = {}): Promise<BackupMetadata> {
    const finalConfig = { ...ENTERPRISE_CONFIG, ...config };
    const timestamp = Date.now();
...
// ============================================================
export async function scheduleBackup(intervalHours: number = 24): Promise<{
    success: boolean;
    message: string;
}> {
    const schedule = {
...
// نسخ احتياطي تلقائي
// ============================================================
export async function autoBackupIfNeeded(): Promise<BackupMetadata | null> {
...
// ============================================================
export async function exportBackup(backupId: string): Promise<{
    success: boolean;
    filename?: string;
}> {
    try {
...
// ============================================================
export async function importBackup(file: File, options: {
    id?: string;
    encrypted?: boolean;
} = {}): Promise<{
    success: boolean;
...
export async function getBackupStats(): Promise<{
    ...
    return {
        ...
        totalSize,
        totalEntities: backups.reduce((acc, b) => acc + b.metadata.entityCount + b.metadata.memberCount +
...
// ============================================================
export async function uploadToCloud(backupId: string, endpoint: string = '/api/backups/upload'): Promise<{
    success: boolean;
    url?: string;
    message?: string;
}> {
...
// استرجاع من السحابة
// ============================================================
export async function downloadFromCloud(backupId: string, cloudUrl: string): Promise<{
    success: boolean;
    message?: string;
}> {
