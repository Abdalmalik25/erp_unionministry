...
router.post('/api/v1/payments', requirePermission('write:payments'), async (req,res)=>{
  try {
    ...
    res.status(500).json({ error: 'خطأ في إنشاء الدفعة', code: 'SERVER_ERROR' });
  }
});
...
router.post('/api/v1/signatures', requirePermission('write:payments'), async (req,res)=>{
  try {
    ...
    console.error('[payments] signature error:', err.message);
    res.status(500).json({ error: 'خطأ في إنشاء التوقيع', code: 'SERVER_ERROR' });
  }
});
