    ...
    <Card>
      <div className="p-5 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm">المساعد الذكي المتقدم — إجابات فورية</span>
          <Badge className="bg-violet-600 text-[10px]"><Sparkles className="w-3 h-3 ml-1"/>أداء متقدم</Badge>
          <Badge variant="outline" className="text-[10px]">بحث موثق • إجابة فورية • خاضع للحوكمة</Badge>
        </div>
        <div className="flex gap-2">
          <input value={q} onChange={e=> setQ(e.target.value)} onKeyDown={e=> e.key==='Enter' && ask()} placeholder="اسأل مع استخدام أدوات: ابحث، طابق سير العمل، تحقق من مدة الإنجاز..." className="flex-1 border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500"/>
        ...
        {stream && (
          <div className="p-4 border rounded-xl bg-gradient-to-br from-violet-50 to-white min-h-[90px]">
            ...
            {done && <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              <Badge variant="outline" className="text-[10px]"><ShieldCheck className="w-3 h-3 ml-1"/>تتطلب مراجعة بشرية</Badge>
              <Badge variant="secondary" className="text-[10px]">الإصدار الرسمي الثاني — درجة الموثوقية 86%</Badge>
            </div>}
          </div>
        )}
