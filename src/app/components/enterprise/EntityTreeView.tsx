...
export function EntityTreeView({ showActions = true, draggable = false, onNodeClick, onDragDrop, }: EntityTreeViewProps) {
    ...
    const getStatusColor = (status: EntityStatus) => {
        switch (status) {
            ...
            case 'under_review':
                return 'text-primary-bright bg-info/10';
            default:
                return 'text-muted-foreground bg-muted';
        }
    };
    const renderTreeNode = (entity: EntityTreeNode, level: number = 0) => {
        ...
        return (<div key={entity.entityId} className="select-none">
        <div className={`
            flex items-center gap-2 px-3 py-2 hover:bg-accent/50 rounded-lg cursor-pointer
            transition-colors duration-150
            ${isSelected ? 'bg-info/10 border border-info/30' : ''}
            ${isDragging ? 'opacity-50' : ''}
          `} style={{ paddingRight: `${level * 24}px` }} onClick={() => handleNodeClick(entity)} draggable={draggable} onDragStart={(e) => handleDragStart(e, entity.entityId)} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, entity.entityId)}>
          {/* Expand/Collapse Button */}
          <button onClick={(e) => {
                e.stopPropagation();
                if (hasChildren)
                    toggleNode(entity.entityId);
            }} className="p-1 hover:bg-accent rounded">
            {hasChildren ? (isExpanded ? (<ChevronDown className="h-4 w-4 text-muted-foreground"/>) : (<ChevronRight className="h-4 w-4 text-muted-foreground"/>)) : (<div className="h-4 w-4"/>)}
          </button>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
              ...
              {entity.branchCount > 0 && (<span>الفروع: {entity.branchCount}</span>)}
              {entity.committeeCount > 0 && (<span>اللجان: {entity.committeeCount}</span>)}
            </div>
          </div>
        ...
        {/* Children */}
        {isExpanded && hasChildren && (<div className="mr-6 border-r border-border">
            {entity.children?.map((child) => renderTreeNode(child, level + 1))}
          </div>)}
      </div>);
    };
    return (<div className="bg-card rounded-lg border border-border p-4">
      {/* Header */}
      <div className="flex items-center justify-between mb-4 pb-4 border-b">
        <h3 className="text-lg font-semibold text-foreground">
          الهيكل التنظيمي
        </h3>
        {showActions && (<button className="flex items-center gap-2 px-3 py-2 bg-primary-bright text-white rounded-lg hover:bg-primary-dark transition-colors">
      ...
      {/* Empty State */}
      {mockEntities.length === 0 && (<div className="text-center py-12 text-muted-foreground">
