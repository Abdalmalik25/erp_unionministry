...
export function OnlineStatusIndicator() {
    const { isOnline, effectiveType } = useOnlineStatus(false);
    if (isOnline) {
        return (<div className="flex items-center gap-2 text-xs text-success-dark">
        <span className="w-2 h-2 bg-success rounded-full animate-pulse"></span>
        <span>متصل</span>
        {effectiveType && (<span className="text-muted-foreground">({effectiveType})</span>)}
      </div>);
    }
    return (<div className="flex items-center gap-2 text-xs text-error bg-error/10 px-3 py-1.5 rounded-full">
...
export function OfflineWarning() {
    ...
    if (isOnline)
        return null;
    return (<div className="fixed bottom-0 left-0 right-0 bg-error text-white px-4 py-3 z-50 shadow-lg" dir="rtl">
      <div className="container mx-auto flex items-center justify-between">
        ...
        <button onClick={() => window.location.reload()} className="px-4 py-2 bg-white text-error rounded-lg font-semibold hover:bg-error/10 transition-colors">
          إعادة المحاولة
        </button>
      </div>
    </div>);
