...
const NATIONAL_REGISTRIES: [string, string][] = [
...
const FAQ: [string, string][] = [
  ...
  ["ماذا لو انقطع الإنترنت أثناء العمل الميداني؟", "تستمر المنصة في العمل بشكل كامل على جهازك: تُحفظ كل عملية في ذاكرة محلية آمنة داخل الجهاز، وعند عودة الاتصال تُزامَن تلقائياً مع الخادم دون أي تدخل، ولا تُحذف أي عملية إلا بعد تأكيد استلام الخادم لها."],
  ["هل يمكن لأحد تعديل سجل التدقيق؟", "مستحيل تقنياً. كل قيد في سجل التدقيق يحمل بصمة رقمية مرتبطة بالقيد الذي قبله؛ أي محاولة تعديل أو حذف تكسر السلسلة وتُكتشف فوراً عند التحقق. الجدول نفسه مقفل ضد التعديل والحذف على مستوى قاعدة البيانات."],
  ["كيف تُحمى بيانات الأشخاص؟", "البيانات الشخصية الحساسة مشفرة بمعيار التشفير المتقدم AES-256، وكلمات المرور تُحفظ بمشتق تشفير scrypt لا يسمح باسترجاعها، والصلاحيات مبنية على مبدأ الحد الأدنى: كل مستخدم يرى ما يتطلبه عمله فقط."],
  ["من يحق له إنشاء الحسابات؟", "إنشاء الحسابات وإسناد الأدوار حكر على إدارة الحسابات بالوزارة. طلبات الانضمام تُقدَّم إلكترونياً وتُراجع وتُعتمد رسمياً، ولا يمكن لأي جهة أخرى منح نفاذ."],
  ["هل تُحتسب درجة خطورة أو عقوبة تلقائياً؟", "لا تُحتسب أي درجة عقوبة إلا بنص نظامي صريح ينظمها، والمنظومة تعرض الأساس النظامي لكل احتساب مع إمكانية مراجعته."],
];

const LEGAL: [string, string][] = [
  ["قانون العمل اليمني", "المرجع الأم لعلاقات العمل: العقود، الأجور، ساعات العمل، الإجازات، إنهاء الخدمة، وتسوية النزاعات — وتفاصيل تنفيذه في اللوائح المصاحبة."],
  ["قانون النقابات والمنظمات", "تنظيم حق التنظيم النقابي: التأسيس، الهياكل، الانتخابات، أموال المنظمات، وعلاقتها الرسمية بالوزارة."],
  ["نسبة التعيين (اليمننة)", "السياسة الوطنية لإيراد العمالة الوطنية؛ نسبة التعيين المعتمدة حالياً 80% وتُطبَّق آلياً على طلبات الترخيص والتوسع وفق القرار الوزاري المنظم."],
  ["أنظمة التفتيش والجزاءات", "إجراءات المعاينة، تصنيف المخالفات، الجزاءات المقابلة لكل مخالفة نصاً، وإجراءات الاعتراض والتظلم."],
];
