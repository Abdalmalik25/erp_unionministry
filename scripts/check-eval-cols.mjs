...
const r = await pool.query("SELECT column_name, udt_name FROM information_schema.columns WHERE table_name='evaluation_certificates' ORDER BY ordinal_position");
console.log('evaluation_certificates columns:', r.rows.map(x=>x.column_name+'('+x.udt_name+')').join(', '));
await pool.end();
